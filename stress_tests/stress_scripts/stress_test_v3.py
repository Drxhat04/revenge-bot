# ────────────────────────────────────────────────────────────────
# stress_test.py  ·  Monte‑Carlo torture‑test  (v3 “shake‑it‑hard”)
# ────────────────────────────────────────────────────────────────
...
ENTRY_SLIDE_RANGE_PCT  = (0.0, 0.40)      # 0‑40 % of zone width
...
def run_single_trial(base_signals, base_bars, base_cfg, trial_no: int):
    ...
    # 4) rolling walk‑forward with balance carry‑over
...
def main():
    ...
    out.to_csv("stress_results_3.csv", index=False)
    ...
    pathlib.Path("stress_summary_3.txt").write_text(summary)
    print(summary)
